...
Open Street Map (OSM) is a crowdsourced project that creates and provides free geographic data, giving everyone the opportunity to create maps of the world. The OSM database holds detailed information about nearly every square meter of the planet’s surface. It includes data on building types, infrastructure, restaurants, street names, parks and even park benches and millions of other data points, all mapped.

It’s worth spending time getting familiar with the basic types of OSM data before starting to build queries. There are three main types of object in the OSM database: nodes, ways, relations.
...
Multipolygons: Used to handle complex areas that consist of multiple polygons, including ones with holes. For example, a forest with several clearings can be represented as a multipolygon where the outer ways define the forest boundary, and inner ways define the clearings. 
Restrictions: Traffic flow restrictions at junctions, such as "no right turn." meaning you can't turn from one way into another way.
